  ...
  return (
    <div className="space-y-6">
      <div className="mb-8 text-center">
        ...
        <h2 className="mb-2 text-2xl font-bold text-foreground">{t('gitStep.title')}</h2>
        <p className="text-muted-foreground">{t('gitStep.description')}</p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label htmlFor="gitName" className="mb-2 flex items-center gap-2 text-sm font-medium text-foreground">
            ...
            {t('gitStep.fields.gitName')} <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            id="gitName"
            ...
            placeholder={t('gitStep.placeholders.gitName')}
            required
            disabled={isSubmitting}
          />
          <p className="mt-1 text-xs text-muted-foreground">{t('gitStep.hints.gitName')}</p>
        </div>

        <div>
          <label htmlFor="gitEmail" className="mb-2 flex items-center gap-2 text-sm font-medium text-foreground">
            <Mail className="h-4 w-4" />
            {t('gitStep.fields.gitEmail')} <span className="text-red-500">*</span>
          </label>
          <input
            type="email"
            id="gitEmail"
            ...
            className="w-full rounded-lg border border-border bg-background px-4 py-3 text-foreground focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder={t('gitStep.placeholders.gitEmail')}
            required
            disabled={isSubmitting}
          />
          <p className="mt-1 text-xs text-muted-foreground">{t('gitStep.hints.gitEmail')}</p>
        </div>
      </div>
    </div>
  );
